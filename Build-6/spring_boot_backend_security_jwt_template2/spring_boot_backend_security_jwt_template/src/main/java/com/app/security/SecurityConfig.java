	...
	public SecurityFilterChain authorizeRequests(HttpSecurity http) throws Exception {
		...
		.authenticationEntryPoint(
			...
			.csrf().disable(). // disable CSRF to continue with REST APIs
				...
				.antMatchers("/api/book/review","/api/user/**","/api/payment","/api/user/carts/**","/api/user/cartQuantUpdate/**/**"
						+ "/api/user/order/**/**","/api/user/update").hasRole("CUSTOMER")// only customer can purchase the products
				.antMatchers("/api/admin/**","/api/admin/**/**", "/api/book/delete/**","/api/book/update/**","/api/category","/api/category/**","/api/category/**/**").hasRole("ADMIN") // only admin can add the products
				.antMatchers(
