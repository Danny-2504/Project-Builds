		...
		.authenticationEntryPoint(
			...
			.csrf().disable(). // disable CSRF to continue with REST APIs
				...
				.antMatchers("/api/book/review","/api/user/**","/api/payment","/api/user/carts/**",""
						...
						,"/api/user/wishlist","/api/user/wishlist/**","/api/user/wishlist/**/**").hasRole("CUSTOMER")// only customer can purchase the products
				.antMatchers("/api/admin/**","/api/admin/**/**", "/api/book/delete/**","/api/book/update/**",""
						+ "/api/category","/api/category/**","/api/category/**/**").hasRole("ADMIN") // only admin can add the products
				.antMatchers(
						"/api/payment/**",
						"/user/signin",
						"/user/signup",
